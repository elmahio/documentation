...
!!! note
    We don't use your log data to train Bugster. Log data is currently only shared with Bugster when clicking the *Analyze with Bugster* button or when asking Bugster about log data specifically. In case you are using an OpenAI-hosted model, your log data may be used to train OpenAI's models, unless you manually disable this in settings. More about that later in this article.
...
On the Bugster configuration, you also need to pick a model. Extended Bugster is currently available on the Business, Business+, and Enterprise plans. Business and Business+ users need to provide their own OpenAI or Gemini API key, while Enterprise users can choose between both an external model and a model hosted by us. By picking an elmah.io-hosted model, you make sure that log data doesn't leave our data center, while the external models will send log data to OpenAI or Gemini. In the latter case, we recommend disabling the *Improve the model for everyone* on OpenAI or disabling *Gemini Apps Activity* on Gemini, to avoid log data being used for training.